...
import { MttModifyColumns } from './child-components/mtt-modify-columns/mtt-modify-columns';
import { MttTruncatedTextDetails } from './child-components/mtt-truncated-text-details/mtt-truncated-text-details';
import { MatSidenav, MatSidenavModule } from '@angular/material/sidenav';
import { debounceTime, map, of, Subject, takeUntil } from 'rxjs';
import { Column } from './models/column.model';
import { MttRowSelectionService } from './services/mtt-row-selection.service';
...
@Component({
  ...
  imports: [
    ...
    MttTruncatedTextDetails,
    PathValuePipe
  ],
  templateUrl: './mtt-table.html',
  styleUrl: './mtt-table.scss',
  providers: [MttRowSelectionService],
  animations: [
    trigger('floatLabel', [
      state('float', style({
...
export class MttTable<T> implements OnChanges, OnInit, OnDestroy {
  /**
   * Table configuration.
   */
  ...
  static nextId = 0;
  protected searchBarId = `app-table-search-bar-${MttTable.nextId++}`;
  ...
  constructor(
    ...
    private rss: MttRowSelectionService<T>,
  ) { }
